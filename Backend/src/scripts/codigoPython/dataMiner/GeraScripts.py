...
dir_base = Path(__file__).resolve().parent  # Isso dá "src/"
dir_meta = str(dir_base) + "/mineracao/META/"
dir_model = str(dir_base) + "/mineracao/model/"
dir_script = str(dir_base) + "/mineracao/script/"
...
def baixar_meta():
    ...
    for nome in nomes:
        ...
        DataMiner.baixar_zips(url,dir_meta,nomes_arquivos)
...
TYPE_MAP = {
    "varchar": lambda size: f"db.String({size})",
    "nvarchar": lambda size: f"db.String({size})",
    "char": lambda size: f"db.String({size})",
...
def gerar_model(schema: str, name:str) -> str:
    ...
    for campo in campos[1:]:
        ...
        colunas.append(
            f"    {nome} = db.Column({coluna_tipo}, nullable=False)"
        )
...
def gerar_script(schema:str, name:str):
    return ""
...
def create_dir_if_dont_exist(categoria: str ):
    ...
    diretorios_script = listdir(dir_model)
    if not categoria in diretorios_model:
        dir = dir_model + f"{categoria}/"
        mkdir(dir) 
    if not categoria in diretorios_script:
        dir = dir_script + f"{categoria}/"
        mkdir(dir) 
...
if __name__ == "__main__":
    ...
    for categoria in metas:
        ...
        for txt in txts:
            ...
            with open(f"{dir_meta}/{categoria}/{txt}",encoding="latin-1") as origem, open(f"{dir_model}/{categoria}/{nome}.py", "w") as model,  open(f"{dir_script}/{categoria}/{nome}.py", "w") as script:
                string_arquivo = origem.read().rstrip()
                model.write(gerar_model(string_arquivo,nome))
                model.write(gerar_script(string_arquivo,nome))
